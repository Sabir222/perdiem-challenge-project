...
The Perdiemin backend is a Node.js + TypeScript API designed for a multi-tenant application with store-specific authentication. It supports subdomain-based store isolation where each subdomain (e.g., a.localhost:4000) maps to a different store with its own user base and data.
